...
const rrhhData = (data.rrhh && Array.isArray(data.rrhh.procesos)) ? data.rrhh.procesos : [];
const neRRHH = seleccion.includes("rrhh") && rrhhData.length > 0;
const apiData = data.api || {};
const neApi = seleccion.includes("api") && ((apiData.otroSoftware || "").trim() || (apiData.flujos || []).length);
...
if (neRRHH) nes.push({ key: "rrhh", label: "Procesos de Gestión de RRHH" });
if (neApi) nes.push({ key: "api", label: "Integración / API" });
...
function renderSubproceso(pipe) {
  ...
  if (Array.isArray(pipe.automatizaciones) && pipe.automatizaciones.length) {
    out.push(tablaX(["Parámetro", "Valor / Configuración"],
      pipe.automatizaciones.map((a) => [a.trigger || a.parametro || "", a.valor != null ? a.valor : (a.acciones || []).map((t) => ({ b: true, t }))]),
      [3000, 6000]));
  } else {
...
const portada = [
  new Paragraph({ alignment: "center", spacing: { before: 200, after: 100 }, children: [
    ...
    new TextRun({ text: "Informe de Consultoría para la Implementación de Necesidad Específica · iCINE", color: COLOR.blue, size: 28, font: FONT, break: 1 })
  ] }),
  ...
  tablaX(["Versión", "Fecha", "Comentario", "Estado", "Realizado por"],
    [[data.meta.version || "1.0", new Date().toLocaleDateString("es"), `Versión ${data.meta.version || "1.0"}`, data.meta.estado, data.meta.consultor || ""]],
...
if (data.incluirTerminos) indiceItems.push("8. Términos y condiciones");
...
if (neReporteria) {
  necesidades.push(tituloNecesidad("Reportería"));
  necesidades.push(parrafo("Documento funcional para la definición de reportes y visualizaciones en BI Builder utilizando información proveniente de Bitrix24. La propuesta considera la estructura de reportes para mostrar la información del CRM de Bitrix24 mediante campos personalizados y nativos, con el fin de dar mayor visibilidad y agilizar la toma de decisiones."));
}
if (neChatbot) {
...
if (neRRHH) {
  necesidades.push(tituloNecesidad("Procesos de Gestión de RRHH"));
  necesidades.push(parrafo(`Contempla la gestión de: ${rrhhData.map((p) => p.label).filter(Boolean).join(", ")}.`));
}
if (neApi) {
  necesidades.push(tituloNecesidad("Integración / API"));
  necesidades.push(parrafo(`Integración entre Bitrix24 y ${(apiData.otroSoftware || "software externo")}.`));
}
necesidades.push(subtitulo("Fuera del alcance de este iCINE (propuesto para Versión 2 / Fase 2)"));
if (data.empresa && (data.empresa.faseDos || "").trim()) necesidades.push(parrafo(data.empresa.faseDos.trim()));
...
const recomendaciones = [
  ...
  ...( (data.empresa && (data.empresa.recomendacionInstancia || "").trim()) ? [subtitulo("Licencia / instancia"), parrafo(data.empresa.recomendacionInstancia.trim())] : [] ),
  subtitulo("Infraestructura / Web Services"),
  parrafo((data.empresa && (data.empresa.webService || "").trim()) ? data.empresa.webService.trim() : "No aplica para el alcance de este iCINE."),
  pageBreak()
];

/* ============================ 4. Diseño ============================ */
// --- NE Captación ---
function consideracionesNE(key) {
  const c = (data.consideraciones && data.consideraciones[key]) ? String(data.consideraciones[key]).trim() : "";
  if (!c) return [];
  return [subtitulo("Consideraciones"), parrafo(c)];
}
function seccionCaptacion() {
  ...
  // Flujo por canal (sub-procesos), desde el formulario
  const flujosCanal = data.captacionDeClientes.flujosPorCanal || {};
  const canalesConFlujo = Object.keys(flujosCanal).filter((k) => (flujosCanal[k] || []).length);
  if (canalesConFlujo.length) {
    out.push(etiqueta(`${N}.5  Flujo por canal`));
    canalesConFlujo.forEach((k) => {
      out.push(tituloSubproceso(`Sub-proceso: Canal ${CANAL_LABELS[k] || k}`));
      out.push(tablaX(["#", "Acción / Descripción", "Responsable", "Herramienta / Módulo", "Condición o Regla"],
        flujosCanal[k].map((p, i) => [String(i + 1), p.accion || "", p.responsable || "", p.herramienta || "", p.condicion || ""]),
        [500, 3100, 1800, 1800, 1800]));
      out.push(espaciador());
    });
    out.push(etiqueta(`${N}.6  Acciones a realizar`));
  } else {
  ...
  out.push(...consideracionesNE("captacion"));
  return out;
}

// --- NE Proceso Comercial (el bloque central, formato completo) ---
function rolesProcesoComercial() {
  const r = (data.procesoComercial && Array.isArray(data.procesoComercial.roles)) ? data.procesoComercial.roles : [];
  if (!r.length) return ROLES_PROCESO_COMERCIAL;
  return r.map((x) => [x.rol || "", (x.permisos || []).join(", "), x.observaciones || ""]);
}
function seccionProcesoComercial() {
  ...
  const out = [cajaNE(`NE ${N}`, "Proceso Comercial", "vista de cliente"),
    ...
    tabla(["Rol", "Permisos en Bitrix24", "Observación"], rolesProcesoComercial(), [2400, 3800, 2800]),
  ...
  out.push(...consideracionesNE("proceso"));
  return out;
}
...
function seccionReporteria() {
  ...
  const out = [cajaNE(`NE ${N}`, "Reportería", "vista de cliente"),
    etiqueta(`${N}.1  ¿Qué se va a implementar?`),
    parrafo("Se implementará un conjunto de reportes e indicadores de gestión sobre Bitrix24, con el objetivo de dar mayor visibilidad y detalle sobre la operación, agilizar la toma de decisiones y permitir un seguimiento oportuno de los resultados. Los reportes consolidan la información del CRM en vistas claras, evitando el trabajo manual de armar planillas y asegurando que cada área acceda a los datos que necesita en tiempo real."),
    ...
    tabla(["Rol", "Acceso", "Observación"], (data.reporteria.roles && data.reporteria.roles.length) ? data.reporteria.roles.map((x) => [x.rol || "", (x.permisos || []).join(", "), x.observaciones || ""]) : ROLES_REPORTERIA, [2800, 4200, 2000]),
    etiqueta(`${N}.4  Flujo del proceso`),
    ...((data.reporteria.descripcionReportes || "").trim()
      ? [parrafo(data.reporteria.descripcionReportes.trim())]
  ...
  const datasets = Array.isArray(data.reporteria.datasets) ? data.reporteria.datasets : [];
  if (datasets.length) {
    out.push(subtitulo("Dataset a crear"));
    datasets.forEach((d, i) => { if ((d.descripcion || "").trim()) out.push(parrafo((datasets.length > 1 ? `Dataset ${i + 1}: ` : "") + d.descripcion.trim())); });
  }
  ...
  let reportes = reportesLista.map((r) => ({
    ...
    queMuestra: r.queMuestra || "",
    entidad: (Array.isArray(r.entidades) ? r.entidades.join(", ") : (r.entidad || "")),
  ...
  reportes.forEach((r) => out.push(...bullets([`Creación del Reporte — ${r.nombre}`])));
  out.push(...consideracionesNE("reporteria"));
  return out;
}
...
function seccionChatbot() {
  ...
  out.push(...consideracionesNE("chatbot"));
  return out;
}
...
// --- NE Procesos de Gestión de RRHH ---
function seccionRRHH() {
  const N = ord.rrhh;
  const out = [
    cajaNE(`NE ${N}`, "Procesos de Gestión de RRHH", "vista de cliente"),
    etiqueta(`${N}.1  ¿Qué se va a implementar?`),
    parrafo(`Se documentarán y automatizarán en Bitrix24 los siguientes procesos de gestión de RRHH: ${rrhhData.map((p) => (p.label || "")).filter(Boolean).join(", ")}.`),
    etiqueta(`${N}.2  Beneficios para tu negocio`),
    tablaX(["✓", "Beneficio", "Descripción"], [
      ["✓", "Solicitudes ordenadas", "Cada pedido sigue un flujo formal y queda registrado."],
      ["✓", "Aprobaciones trazables", "Se sabe quién aprobó, cuándo y en qué orden."],
      ["✓", "Menos gestión manual", "Se reduce el ida y vuelta por correo o papel."]
    ], [700, 3100, 5200]),
    aviso("EDITAR: personalizar los beneficios según el cliente."),
    etiqueta(`${N}.3  Diagrama del proceso:`),
    aviso("Diagrama a insertar aparte — no se genera en este documento."),
    espaciador(300),
    cajaNE(`NE ${N}`, "DETALLE TÉCNICO", "Uso interno del equipo — cómo se va a construir"),
    etiqueta(`${N}.4  Procesos a configurar`)
  ];

  rrhhData.forEach((pr) => {
    out.push(subtitulo("🔧 " + (pr.label || "Proceso")));
    const filas = (pr.campos || []).map((c) => [c.label, c.valor || "(a completar)"]);
    if (Array.isArray(pr.aprobadores) && pr.aprobadores.length) {
      filas.push(["Aprueban (en orden)", pr.aprobadores.map((a, i) => `${i + 1}. ${a}`).join("   →   ")]);
    }
    out.push(tablaX(["Parámetro", "Valor / Configuración"], filas.length ? filas : [["", "(a completar)"]], [3200, 5800]));
    out.push(espaciador());
  });

  out.push(etiqueta(`${N}.5  Acciones a realizar`));
  rrhhData.forEach((pr) => out.push(...bullets([`Configuración del proceso: ${pr.label}`])));
  out.push(...bullets(["Definición de formularios de solicitud", "Configuración de las etapas de aprobación", "Permisos y notificaciones"]));
  out.push(...consideracionesNE("rrhh"));
  return out;
}

// Bloque fijo de consideraciones y responsabilidades — se imprime en API y App.
function consideracionesIntegracion(numero) {
  return [
    etiqueta(`${numero}  Consideraciones y responsabilidades`),
    ...bullets([
      "El desarrollo se realiza y funciona bajo los estándares y la API oficial de Bitrix24. Cualquier cambio en los métodos, respuestas o disponibilidad de la plataforma Bitrix24 escapa al control de asesores-e y puede requerir una actualización.",
      "Del lado del software externo, la integración se ajusta a lo que su documentación / API indique al momento del desarrollo. Cambios posteriores en ese software (métodos, formatos, disponibilidad) no son responsabilidad de asesores-e y pueden requerir un nuevo desarrollo.",
      "La sincronización se basa en el mapeo de identificadores (IDs) entre ambos sistemas; su integridad depende de que ambos sistemas mantengan sus registros y códigos correspondientes.",
      "Los tiempos de respuesta y la disponibilidad dependen también de la infraestructura y los límites de uso (rate limits) de ambas plataformas."
    ])
  ];
}
...
function seccionAPI() {
  const N = ord.api;
  const a = apiData;
  const soft = (a.otroSoftware || "").trim() || "(software a definir)";
  const dirs = [];
  if (a.dirBitrixHaciaOtro) dirs.push("Bitrix24 → " + soft);
  if (a.dirOtroHaciaBitrix) dirs.push(soft + " → Bitrix24");
  const dirTxt = dirs.length ? dirs.join(" y ") : "(dirección a definir)";

  const out = [
    cajaNE(`NE ${N}`, "Integración / API", "vista de cliente"),
    etiqueta(`${N}.1  ¿Qué se va a implementar?`),
    parrafo(`Se desarrollará una integración entre Bitrix24 y ${soft}. Sentido de la integración: ${dirTxt}.`),
    etiqueta(`${N}.2  Beneficios para tu negocio`),
    tablaX(["✓", "Beneficio", "Descripción"], [
      ["✓", "Datos sincronizados", "La información se mantiene consistente entre ambos sistemas."],
      ["✓", "Menos carga manual", "Se evita cargar lo mismo dos veces en cada plataforma."],
      ["✓", "Trazabilidad", "Cada registro queda vinculado por su identificador en ambos lados."]
    ], [700, 3100, 5200]),
    aviso("EDITAR: personalizar los beneficios según el cliente."),
    etiqueta(`${N}.3  Diagrama del proceso:`),
    aviso("Diagrama de la integración a insertar aparte — no se genera en este documento."),
    espaciador(300),
    cajaNE(`NE ${N}`, "DETALLE TÉCNICO", "Uso interno del equipo — cómo se va a construir"),
    etiqueta(`${N}.4  Arquitectura de la integración`),
    tablaX(["Parámetro", "Valor / Configuración"], [
      ["Software externo", soft],
      ["Sentido", dirTxt],
      ["Web service / API propia", a.webService ? ("Sí" + (a.webServiceNota ? " — " + a.webServiceNota : "")) : "No"]
    ], [3000, 6000]),
    parrafo("El mecanismo base es el mapeo de identificadores: cuando se crea un registro en un sistema, el otro devuelve un ID que se almacena para que las futuras actualizaciones sepan a qué registro corresponden en cada lado."),
    etiqueta(`${N}.5  Flujos de sincronización`)
  ];

  const DIR_LABEL = { desde_bitrix: "Bitrix24 → externo", hacia_bitrix: "Externo → Bitrix24", ambos: "Ambos sentidos" };
  const flujos = Array.isArray(a.flujos) ? a.flujos : [];
  if (!flujos.length) {
    out.push(aviso("EDITAR: definir los flujos de sincronización (qué objeto, en qué sentido, cómo)."));
  } else {
    out.push(tablaX(["Objeto", "Sentido", "Descripción del flujo", "Mapea ID"],
      flujos.map((f) => [
        f.objeto || "",
        DIR_LABEL[f.direccion] || "",
        (f.descripcion || "") + (f.consideraciones ? `  (${f.consideraciones})` : ""),
        f.mapeaId ? "Sí" : "—"
      ]), [1700, 2100, 4400, 800]));
  }

  if (a.sincronizaProductos) {
    out.push(etiqueta(`${N}.6  Sincronización de productos`));
    out.push(parrafo("La integración requiere sincronización de productos. " + (a.codigoProductos || "Cada producto debe identificarse mediante un código común en ambos sistemas.")));
    out.push(aviso("Si se crea un producto nuevo en un sistema, debe crearse también en el otro (aunque sea manualmente) con el código correspondiente."));
  }

  out.push(...consideracionesIntegracion(`${N}.7`));
  out.push(etiqueta(`${N}.8  Acciones a realizar`));
  out.push(...bullets([
    "Análisis de la API del software externo",
    "Desarrollo del web service / conectores",
    "Configuración del mapeo de identificadores",
    a.sincronizaProductos ? "Definición del código común de productos y su sincronización" : "Definición de los objetos a sincronizar",
    "Pruebas de sincronización en ambos sentidos"
  ]));
  out.push(...consideracionesNE("api"));
  return out;
}
...
if (neRRHH) diseno.push(...seccionRRHH(), pageBreak());
if (neApi) diseno.push(...seccionAPI(), pageBreak());
...
function resumenActividades() {
  ...
  if (neCaptacion) {
    rows.push(["", "Captación", "", "", ""]);
    rows.push(["", "", "Configuración de canales / Contact Center.", "[  ]", "No"]);
  }
  if (neReporteria) {
    rows.push(["", "Reportería", "", "", ""]);
    const dss = Array.isArray(data.reporteria.datasets) ? data.reporteria.datasets : [];
    if (dss.length) {
      dss.forEach((d, i) => rows.push(["", "", `Creación del Dataset${dss.length > 1 ? " " + (i + 1) : ""} y permisos en BI Builder.`, (d.tiempo || "").trim() || "[  ]", "No"]));
    } else {
      rows.push(["", "", "Creación del Dataset y permisos en BI Builder.", "[  ]", "No"]);
    }
    reportesLista.forEach((r) => rows.push(["", "", `Creación del Reporte — ${r.nombre || "Reporte"}.`, (r.tiempo || "").trim() || "[  ]", "No"]));
  }
  if (neChatbot) {
    rows.push(["", "Chatbot", "", "", ""]);
    rows.push(["", "", "Construcción y conexión del bot con Bitrix24.", "[  ]", "No"]);
  }
  rows.push(["", "Pruebas", "", "", ""]);
  rows.push(["", "", "Pruebas internas QA.", (data.pruebas && data.pruebas.qa || "").trim() || "[  ]", "No"]);
  rows.push(["", "", "Pruebas Alpha con el cliente.", (data.pruebas && data.pruebas.alpha || "").trim() || "[  ]", "Sí"]);
  rows.push(["", "Capacitación — CIP", "", "", ""]);
  const cipData = data.cip || {};
  const cipTodas = Object.values(cipData).flat().filter((c) => c && ((c.titulo || "").trim() || (c.tiempo || "").trim()));
  if (cipTodas.length) {
    cipTodas.forEach((c) => rows.push(["", "", c.titulo || "Capacitación", (c.tiempo || "").trim() || "[  ]", "Sí"]));
  } else {
    rows.push(["", "", "Capacitación al cliente (CIP).", "[  ]", "Sí"]);
  }
  let n = 0;
...
const glosario = [seccionH1("6", "GLOSARIO"),
...
const terminos = data.incluirTerminos ? [
  pageBreak(),
  seccionH1("8", "TÉRMINOS Y CONDICIONES"),
  ...bullets([
...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      ...diseno, ...resumen, ...glosario, ...firmas, ...terminos
    ]
  }]
});
