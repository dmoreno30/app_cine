...
// El Chatbot se activa por la selección de desarrollos (nuevo) o por el toggle
// viejo de captación (compatibilidad hacia atrás con JSONs anteriores).
...
if (neChatbot) recAdicionales.push(["Chatbot", chatbotData.plataforma || "Proveedor externo (a definir)", "Pago por licencia", chatbotData.objetivo || data.captacionDeClientes.chatbot.descripcion || ""]);
...
function seccionChatbot() {
  ...
  const plataforma = cb.plataforma || "(plataforma a definir)";
  ...
  const out = [
    ...
    etiqueta(`${N}.1  ¿Qué se va a implementar?`),
    parrafo(`Se documentará el flujo de ${tipoTxt} sobre ${plataforma}, vinculado a Bitrix24. Objetivo: ${objetivo}`),
    etiqueta(`${N}.2  Beneficios para tu negocio`),
    tablaX(["✓", "Beneficio", "Descripción"], [
    ...
    espaciador(300),
    cajaNE(`NE ${N}`, "DETALLE TÉCNICO", "Uso interno del equipo — cómo se va a construir"),
    etiqueta(`${N}.4  Plataforma`),
    parrafo(`El bot corre sobre: ${plataforma}. Es una plataforma externa vinculada a Bitrix24 (requiere licencia propia).`),
    aviso("Requiere licencia externa de pago — se cotiza aparte según proveedor y cantidad de flujos.")
  ];
