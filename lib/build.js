...
function seccionCaptacion() {
  ...
  out.push(aviso("EDITAR: definir roles de Contact Center — no capturado en el formulario."));
  return out;
}
...
function seccionEntidadPipeline(key) {
  ...
  if (ent.flujo) {
    ...
    out.push(parrafo(ent.flujo));
    out.push(aviso("EDITAR: expandir esta narrativa a la tabla de pasos (acción / responsable / herramienta / condición)."));
  }
  if (ent.automatizacion) {
    out.push(subtitulo(`Automatización — ${ENTIDAD_LABELS[key]}`));
    out.push(parrafo(ent.automatizacion));
  }
...
function seccionProcesoComercial() {
  ...
  out.push(subtitulo("DETALLE TÉCNICO"));
  out.push(etiqueta("Roles y permisos"));
  out.push(aviso("EDITAR: definir roles de CRM — no capturado en el formulario."));
...
function seccionReporteria() {
  ...
  out.push(subtitulo("DETALLE TÉCNICO"));
  const filas = [...data.reporteria.prospectos.map((k) => ["Prospectos", REPORTE_LABELS[k]]), ...data.reporteria.negociaciones.map((k) => ["Negociaciones", REPORTE_LABELS[k]])];
  if (filas.length) out.push(tabla(["Entidad", "Reporte"], filas, [3000, 6000]));
  if (data.reporteria.otros) out.push(parrafo(`Reporte adicional solicitado: ${data.reporteria.otros}`));
  return out;
}
