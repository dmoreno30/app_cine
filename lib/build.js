...
if (neReporteria) {
  ...
  necesidades.push(parrafo("Documento funcional para la definición de reportes y visualizaciones en BI Builder utilizando información proveniente de Bitrix24. La propuesta considera la estructura de reportes para mostrar la información correspondiente (tareas, negociaciones, prospectos, etc.) mediante campos personalizados y nativos, con el fin de dar mayor visibilidad y agilizar la toma de decisiones."));
...
const recomendaciones = [
  ...
  parrafo((data.empresa && (data.empresa.webService || "").trim()) ? data.empresa.webService.trim() : "No aplica para el alcance de este iCINE, salvo indicación contraria."),
  pageBreak()
];
...
function seccionReporteria() {
  ...
  if ((data.reporteria.dataset || "").trim()) {
    out.push(subtitulo("Dataset a crear"));
    out.push(parrafo(data.reporteria.dataset.trim()));
  }
...
function resumenActividades() {
  ...
  if (neReporteria) {
    ...
    reportesLista.forEach((r, i) => rows.push(["", "", `Configuración del Reporte ${i + 1} — ${r.nombre || "Reporte"}.`, (r.tiempo || "").trim() || "[  ]", "No"]));
  }
  if (neChatbot) {
    rows.push(["", "Chatbot", "", "", ""]);
