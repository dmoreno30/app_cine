    ...
    children: lines.map((ln) => {
      ...
      return new Paragraph({
        ...
        children: [new TextRun({
          ...
          color: opts.header ? COLOR.blue : (opts.whiteText ? COLOR.white : (opts.group ? COLOR.redDark : undefined)),
...
function cajaNE(nLabel, titulo, sub) {
  const cell = (txt, w) => new TableCell({
    width: { size: w, type: WidthType.DXA }, shading: { type: ShadingType.CLEAR, fill: COLOR.grayLight },
    margins: { top: 60, bottom: 60, left: 120, right: 120 },
    children: (Array.isArray(txt) ? txt : [txt]).map((t, i) =>
      new Paragraph({ children: [new TextRun({ text: t, bold: i === 0, color: COLOR.redDark, size: i === 0 ? 24 : 20, font: FONT })] }))
  });
  return new Table({
...
function bannerHoras() {
  const W = [3600, 1800, 1800, 1800];
  const header = new TableRow({ tableHeader: true, children:
    ["Necesidad Específica", "Horas de desarrollo", "Horas de capacitación", "Total horas"].map((h, i) =>
      celdaX(h, { width: W[i], fill: COLOR.red, whiteText: true, boldAll: true })) });
  const body = nes.map((n) => new TableRow({ children:
    [n.label, "[  ] hrs", "[  ] hrs", "[  ] hrs"].map((v, i) => celdaX(v, { width: W[i] })) }));
  body.push(new TableRow({ children:
    ["TOTAL", "[  ] hrs", "[  ] hrs", "[  ] hrs"].map((v, i) => celdaX(v, { width: W[i], fill: COLOR.grayLight, boldAll: true })) }));
  return new Table({ width: { size: 9000, type: WidthType.DXA }, columnWidths: W, rows: [header, ...body] });
}
const portada = [
...
const indice = [seccionH2("ÍNDICE DE CONTENIDOS"), tablaX(["Sección"], indiceItems.map((i) => [i]), [9000]), pageBreak()];
...
if (neCaptacion) {
  necesidades.push(seccionH2("[NE] Proceso de Captación y Gestión de Clientes (Omnicanalidad)"));
...
necesidades.push(seccionH2("[NE] Proceso Comercial"));
...
if (neReporteria) {
  necesidades.push(seccionH2("[NE] Reportería y Análisis Comercial"));
...
if (neChatbot) {
  necesidades.push(seccionH2("[NE] Proceso de Atención mediante Chatbot"));
...
function seccionReporteria() {
  ...
  if (data.reporteria.otros) {
    ...
    out.push(aviso("EDITAR: definir Qué muestra / Entidad / Filtros clave / Tipo de visualización para este reporte."));
  }
  ...
  seleccionados.forEach(({ key, meta }, i) => out.push(...bullets([`Configuración del Reporte ${i + 1} — ${meta ? meta.label : key}`])));
...
function seccionChatbot() {
  const N = ord.chatbot;
  return [cajaNE(`NE ${N}`, "Proceso de Atención mediante Chatbot", "vista de cliente"),
    etiqueta(`${N}.1  ¿Qué se va a implementar?`),
    parrafo(data.captacionDeClientes.chatbot.descripcion || "(sin detalle del cliente)"),
    aviso("Requiere licencia externa de pago — se recomienda cotizar aparte según proveedor y cantidad de flujos."),
    cajaNE(`NE ${N}`, "DETALLE TÉCNICO", "Uso interno del equipo — cómo se va a construir"),
    etiqueta(`${N}.2  Definición del bot`),
    aviso("EDITAR: definir flujos, mensajes y lógica de derivación — no capturado en el formulario.")];
}
...
function resumenActividades() {
  ...
  if (neReporteria) {
    ...
    const total = data.reporteria.prospectos.length + data.reporteria.negociaciones.length + (data.reporteria.otros ? 1 : 0);
